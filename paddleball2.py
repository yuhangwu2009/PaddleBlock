    ...
    def hit_block_bottom(self, pos):
        ...
        while x < blocks:
            if self.block_list[x].shown:
                ...
                if pos[0] <= block_pos[2] <= pos[2]:
                    if pos[1] <= block_pos[3] <= pos[3]:
                        self.y = 1
                        block_list[x].hitted()
                        # self.score.hit()
                        return True
...
class Block:
    ...
    def hitted(self):
        ...
        if self.color == 'silver':
            ...
            self.draw()
            #canvas.delete(self.id)
            #canvas.create_rectangle(self.x1, self.y1, self.width, self.height, fill=self.color)
        if self.color == 'gold':
            canvas.delete(self.id)
            canvas.create_rectangle(self.x1, self.y1, self.width, self.height, fill='silver')
        if self.color == 'red':
            ball2 = Ball(canvas, paddle, 'red', block_list)
            ball2.draw()
            canvas.delete(self.id)
            canvas.create_rectangle(self.x1, self.y1, self.width, self.height, fill='gold')
